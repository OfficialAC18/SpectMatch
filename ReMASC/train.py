...
from dataset.remasc_spectrogram import REMASC_DSET_GETTER_1
from dataset.remasc_AST import REMASC_GETTERS_2
from dataset.remasc_3_channels import REMASC_GETTERS_3
...
def main():
    ...
    parser.add_argument('--dataset', default='cifar10', type=str,
                        choices=['cifar10', 'cifar100','classaudio','classaudio_2','classaudio_spectrogram', 'classaudio_AST','classaudio_3_channels', 'classaudio_AudioMAE','remasc_spectrogram','remasc_AST', 'remasc_3_channels'],
                        help='dataset name')
    ...
    elif args.dataset in ['remasc_spectrogram','remasc_AST','remasc_3_channels']:
        args.num_classes = 2
        if args.arch == 'wideresnet':
            args.model_depth = 28
            args.model_width = 2
    ...
    if(args.dataset not in ['classaudio','classaudio_2','classaudio_spectrogram','classaudio_AST','classaudio_3_channels','classaudio_AudioMAE','remasc_spectrogram','remasc_AST','remasc_3_channels']):
        ...
        args)
    elif(args.dataset == 'remasc_spectrogram'):
    	labeled_dataset, unlabeled_dataset, test_dataset = REMASC_DSET_GETTER_1[args.dataset](
    	args)
    elif(args.dataset == 'remasc_AST'):
    	labeled_dataset, unlabeled_dataset, test_dataset = REMASC_GETTERS_2[args.dataset](
    	args)
    elif(args.dataset == 'remasc_3_channels'):
    	labeled_dataset, unlabeled_dataset, test_dataset = REMASC_GETTERS_3[args.dataset](
    	args)
...
def test(args, test_loader, model, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            data_time.update(time.time() - end)
            model.eval()
            if (args.dataset in ['classaudio_3_channels','remasc_3_channels']):
