...
class ASTModel(nn.Module):
  ...
  def __init__(self, label_dim = 4, fstride = 10, tstride = 10, input_fdim = 128, input_tdim = 298, imagenet_pretrain = True, audioset_pretrain = False, model_size = 'base384'):
    ...
    if audioset_pretrain == False:
      ...
      audio_model = ASTModel(label_dim=4, fstride=10, tstride=10, input_fdim=128, input_tdim=100, imagenet_pretrain=False, audioset_pretrain=False, model_size='base384', verbose=False)
  ...
  def forward(self, x):
    ...
    '''
    if(count == 0):
      self.v.pos_embed.data = torch.unsqueeze(self.v.pos_embed.data, 0)
      self.v.pos_embed.data = F.interpolate(self.v.pos_embed.data, [470,768], mode = 'bilinear',align_corners = True)
      self.v.pos_embed.data = torch.squeeze(self.v.pos_embed, 0)
      count+=1
    '''
    ...
    x = x + self.v.pos_embed#[:,:290,:]#[:,:230,:]
    #x = x + torch.squeeze(F.interpolate(torch.unsqueeze(self.v.pos_embed.unsqueeze,0), [470,128], mode='bilinear', align_corners = True),0)#[:,:230,:]
    x = self.v.pos_drop(x)
